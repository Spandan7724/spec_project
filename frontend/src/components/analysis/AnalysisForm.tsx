...
export default function AnalysisForm() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 border rounded-lg p-4 md:p-6">
        {/* Currency Pair */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              className="mobile-tap w-full px-3 py-3 md:py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-card text-foreground border-input text-base"
            >
        ...
        <div>
          ...
          <input
            ...
            min="0"
            step="0.01"
            className="mobile-tap w-full px-3 py-3 md:py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-card text-foreground border-input placeholder:text-muted-foreground text-base"
          />
        </div>
        ...
        <button
          ...
          className="mobile-tap w-full py-3 md:py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2 font-medium text-base"
        >
          {isSubmitting ? (
            <>
      ...
      {recentAnalyses.length > 0 && (
        <div className="border rounded-lg p-4 md:p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg md:text-xl font-semibold">Recent Analyses</h2>
            <Link to="/history" className="mobile-tap text-sm text-primary hover:underline">
              View all
            </Link>
          </div>
          <ul className="space-y-3">
            {recentAnalyses.map((item) => (
              <li key={item.correlationId} className="flex items-center justify-between gap-2 md:gap-4 border rounded-md px-3 py-3 md:py-2">
                ...
                <Link
                  ...
                  className="mobile-tap text-sm font-medium text-primary hover:underline flex-shrink-0"
                >
                  Open
                </Link>
              </li>
