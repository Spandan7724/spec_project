...
app = typer.Typer(add_completion=False, help="Model training CLI")
...
def train_model(
    ...
    typer.echo(f"Training LightGBM for {pair} (days={days}, horizons={horizons})")
    try:
        meta = asyncio.run(
            train_and_register_lightgbm(
                pair,
        ...
        typer.echo(f"Registry: {cfg.model_registry_path}\nStorage: {cfg.model_storage_dir}")
...
def train_lstm(
    ...
    horizons_hours: Optional[List[int]] = typer.Option(
        None, "--horizon-hours", "-H", help="Intraday horizons in hours (repeat)"
    ),
    version: str = typer.Option("1.0", "--version", "-v", help="Model version label"),
    # LSTM knobs
    ...
    typer.echo(
        f"Training LSTM for {pair} (days={days}, interval={interval}, horizons_hours={horizons_hours})"
    )
    try:
        meta = asyncio.run(
            train_and_register_lstm(
